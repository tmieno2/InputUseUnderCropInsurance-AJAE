...
MP_gen <- function(i){
	...
	for (k in 1:ui_pairs_len){
		...
		# search_N[which.max(MP_data[,tot_prof])]
...
#--------------------------
search_N[which.max(MP_data[coverage==0.85 & insurance == 'RP' & utility=='RN' & subsidy==0.38,tot_prof])]
search_N[which.max(MP_data[coverage==0.70 & insurance == 'RP' & utility=='RN' & subsidy==0.59,tot_prof])]
