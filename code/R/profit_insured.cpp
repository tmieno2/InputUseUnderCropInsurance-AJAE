...
double premium_calc_RPHPE(
	...
	if (cov_level>=0.65){
		...
		(beta_RPHPE(0) 
			+ round(beta_RPHPE(1)*hisotircal_basic_unit_base_rate*1e8)/1e8 
			+ round(beta_RPHPE(2)*pow(hisotircal_basic_unit_base_rate,2)*1e8)/1e8
			+ round(beta_RPHPE(3)*cov_level*1e8)/1e8
			+ round(beta_RPHPE(4)*pow(cov_level,2)*1e8)/1e8
			+ round(beta_RPHPE(5)*(APH/capping_reference_yield)*1e8)/1e8
			+ round(beta_RPHPE(6)*pow((APH/capping_reference_yield),2)*1e8)/1e8
			+ round(beta_RPHPE(7)*pvf*1e8)/1e8
			+ round(beta_RPHPE(8)*pow(pvf,2)*1e8)/1e8
			+ round(beta_RPHPE(9)*hisotircal_basic_unit_base_rate*cov_level*1e8)/1e8
			+ round(beta_RPHPE(10)*hisotircal_basic_unit_base_rate*(APH/capping_reference_yield)*1e8)/1e8
			+ round(beta_RPHPE(11)*hisotircal_basic_unit_base_rate*pvf*1e8)/1e8
			+ round(beta_RPHPE(12)*cov_level*(APH/capping_reference_yield)*1e8)/1e8
			+ round(beta_RPHPE(13)*cov_level*pvf*1e8)/1e8
			+ round(beta_RPHPE(14)*(APH/capping_reference_yield)*pvf*1e8)/1e8
		)*c_pars(2)*1.1;
...
double premium_calc_RP(
	...
	if (cov_level>=0.65){
		...
		double historical_revenue_protection_base_premium_rate = 
		(beta_RP(0) 
			+ round(beta_RP(1)*hisotircal_basic_unit_base_rate*1e8)/1e8 
			+ round(beta_RP(2)*pow(hisotircal_basic_unit_base_rate,2)*1e8)/1e8
			+ round(beta_RP(3)*cov_level*1e8)/1e8
			+ round(beta_RP(4)*pow(cov_level,2)*1e8)/1e8
			+ round(beta_RP(5)*(APH/capping_reference_yield)*1e8)/1e8
			+ round(beta_RP(6)*pow((APH/capping_reference_yield),2)*1e8)/1e8
			+ round(beta_RP(7)*pvf*1e8)/1e8
			+ round(beta_RP(8)*pow(pvf,2)*1e8)/1e8
			+ round(beta_RP(9)*hisotircal_basic_unit_base_rate*cov_level*1e8)/1e8
			+ round(beta_RP(10)*hisotircal_basic_unit_base_rate*(APH/capping_reference_yield)*1e8)/1e8
			+ round(beta_RP(11)*hisotircal_basic_unit_base_rate*pvf*1e8)/1e8
			+ round(beta_RP(12)*cov_level*(APH/capping_reference_yield)*1e8)/1e8
			+ round(beta_RP(13)*cov_level*pvf*1e8)/1e8
			+ round(beta_RP(14)*(APH/capping_reference_yield)*pvf*1e8)/1e8
		)*c_pars(2)*1.1;
...
// [[Rcpp::export]]
List pi_insured(
	double APH, double rate_yield, int B, arma::vec mu, arma::mat sigma, 
	...
	arma::vec c_pars, arma::vec beta_RPHPE, arma::vec beta_RP, arma::vec sec5, arma::vec sec6
	)
{
	//------------------
	// Glossary
	...
	// //--- revenue guarantee under RP (MWF) ---//
	// NumericVector revenue_guarantee_RP = APH*cov_level*acres*pmax(hp,pp);

	// //--- revenue guarantee under RP (BH) ---//
	// NumericVector revenue_guarantee_RPHPE = APH*cov_level*acres*pp;
	...
	NumericVector revenue_RP = pmax(raw_revenue,APH*cov_level*acres*pmax(hp,pp));

	// //------------------
	// // Indemnity
	// //------------------
	// NumericVector indemnity_YP = pmax(revenue_YP-raw_revenue,0);
	// NumericVector indemnity_RPHPE = pmax(revenue_RPHPE-raw_revenue,0);
	// NumericVector indemnity_RP = pmax(revenue_RP-raw_revenue,0);

	//------------------
	// Profit
	//------------------
	NumericVector profit_YP = revenue_YP - cost - premium_YP; // premium is fixed
	NumericVector profit_RPHPE = revenue_RPHPE - cost - premium_RPHPE; // premium is fixed
	NumericVector profit_RP = revenue_RP - cost - premium_RP; // premium is fixed
	...
	return Rcpp::List::create(
		...
		// Rcpp::Named("indemnity_YP")=indemnity_YP,
		// Rcpp::Named("indemnity_RPHPE")=indemnity_RPHPE,
		// Rcpp::Named("indemnity_RP")=indemnity_RP,
		// Rcpp::Named("revenue_YP")=revenue_YP,
		// Rcpp::Named("revenue_RPHPE")=revenue_RPHPE,
		// Rcpp::Named("revenue_RP")=revenue_RP,
		// Rcpp::Named("raw_revenue")=raw_revenue,
		Rcpp::Named("h_price")=hp,
		Rcpp::Named("premium_YP")=premium_YP,
		Rcpp::Named("premium_RPHPE")=premium_RPHPE,
		Rcpp::Named("premium_RP")=premium_RP,
